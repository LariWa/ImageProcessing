...
public class ImageAnalysisAppController {
	...
	public enum StatsProperty {
		...
		public void setValue(double value) {
			this.value.set(String.format("%.2f", value));
		}

		public void setValue(int value) {
			this.value.set(String.format("%d", value));
		}
	}
	...
	void contrastChanged() {
		...
		contrastLabel.setText("" + contrast / 10);
		processImage();
	}
	...
	void AutoContrast() {
		...
		gammaChanged();
		processImage();
		int min = histogram.getMinimum();
		int max = histogram.getMaximum();
		...
		if(max-min !=0)
		mittel=mittel/(max-min);
		else
			mittel = min;
		//double mittel = histogram.getMittelwert(min, max);
		if (mittel != 128)
			brightness = -((int) mittel - 128);
	...
	void selectionResized(MouseEvent event) {
		...
		if (ey < 0)
			ey = 0;
		if (ex >= originalImage.width)
			ex = originalImage.width - 1;
		if (ey >= originalImage.height)
			ey = originalImage.height - 1;
